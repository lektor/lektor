...
def test_child_sources_basic(pad, builder):
    ...
    assert [x["_id"] for x in child_sources] == ["a", "b", "hello.txt"]
...
def test_child_sources_pagination(pad, builder):
    ...
    assert ids(child_sources_p1) == ["bagpipe", "coffee", "master", "oven"]

    prog, _ = builder.build(page2)
    child_sources_p2 = get_child_sources(prog)

    assert ids(child_sources_p2) == ["postage", "slave", "wolf"]
...
def test_basic_artifact_current_test(pad, builder, reporter):
    ...
    assert reporter.get_major_events() == [
        ("enter-source", {"source": post1}),
        ("start-artifact-build", {"artifact": artifact, "is_current": False}),
        ("build-func", {"func": "lektor.build_programs.PageBuildProgram"}),
        ("finish-artifact-build", {"artifact": artifact}),
        ("leave-source", {"source": post1}),
    ]
    ...
    assert reporter.get_major_events() == [
        ("enter-source", {"source": post1}),
        ("start-artifact-build", {"artifact": artifact, "is_current": True}),
        ("build-func", {"func": "lektor.build_programs.PageBuildProgram"}),
        ("finish-artifact-build", {"artifact": artifact}),
        ("leave-source", {"source": post1}),
    ]
